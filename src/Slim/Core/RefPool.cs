...
namespace Slim.Core
{
  internal class RefPool
  {
    ...
    /// </summary>
    public MetaHandle GetHandle(object reference, TypeRegistry typeRegistry, SlimFormat format, out Type type,
      bool serializationForFrameWork)
    {
      ...
      if (reference is Type typeRef)
      {
        var typeHandle = typeRegistry.GetTypeHandle(typeRef, serializationForFrameWork);
        return MetaHandle.InlineTypeValue(typeHandle);
      }


      if (type.IsValueType)
      {
        var vth = typeRegistry.GetTypeHandle(type, serializationForFrameWork);
      ...
      var th = typeRegistry.GetTypeHandle(type, serializationForFrameWork);
    ...
    /// </summary>
    public object HandleToReference(MetaHandle handle, TypeRegistry typeRegistry, SlimFormat format, SlimReader reader)
    {
      ...
      {
        var typeRef = typeRegistry.GetOrAddType(handle.Metadata.Value); //adding this type to registry if it is not there yet
        return typeRef;
      }
      ...
      if (handle.IsInlinedRefType)
      {
        ...
        throw new SlimException(
          "Internal error HandleToReference: no read action for ref type, but ref metaHandle is inlined");
      }


      var idx = handle.Handle;
      if (idx < m_List.Count) return m_List[idx];

      if (!handle.Metadata.HasValue)
        throw new SlimException(StringConsts.HandleToRefMissingTypeNameError + handle);
      ...
      if (metadata.StringValue != null) //need to search for possible array descriptor
      {
        ...
        else
        {
          ...
          type = typeRegistry.GetOrAddType(metadata);
        }
      }
      else
      {
    ...
    private int GetIndex(object reference, out bool added)
    {
      ...
      if (cnt < maxLinearSearch)
      {
        for (var i = 1; i < cnt; i++) //start form 1, skip NULL[0]
          if (ReferenceEquals(m_List[i], reference))
            return i;
      }
