  ...
  internal static class Arrays
    {
          ...
          public const int MAX_ELM_COUNT = MAX_DIM_COUNT * 10 * 1024 * 1024;
           ...
           public static string ArrayToDescriptor(Array array, Type type, VarIntStr typeHandle)
           {
              if (array.LongLength>MAX_ELM_COUNT)
                throw new SlimException(StringConsts.SLIM_ARRAYS_OVER_MAX_ELM_ERROR.Args(array.LongLength, MAX_ELM_COUNT));
              ...
              var th = typeHandle.StringValue ??
               ...
               var ar = array.Rank;
               if (ar>MAX_DIM_COUNT)
                throw new SlimException(StringConsts.SLIM_ARRAYS_OVER_MAX_DIMS_ERROR.Args(ar, MAX_DIM_COUNT));
      ...
      public static Array DescriptorToArray(string descr, Type type)
      {
          ...
          if (i<2)
          {
            throw new SlimException(StringConsts.SLIM_ARRAYS_MISSING_ARRAY_DIMS_ERROR + descr.ToString());
          }
          ...
          if (i==2 && descr[1]=='2' && descr[0]=='$')//object[] case: $2|len
          {
            ...
            if (total>MAX_ELM_COUNT)
              throw new SlimException(StringConsts.SLIM_ARRAYS_OVER_MAX_ELM_ERROR.Args(total, MAX_ELM_COUNT));
          ...
          if (!type.IsArray)
            throw new SlimException(StringConsts.SLIM_ARRAYS_TYPE_NOT_ARRAY_ERROR + type.FullName);
          ...
          try
          {
              ...
              if (dimCount>MAX_DIM_COUNT)
                throw new SlimException(StringConsts.SLIM_ARRAYS_OVER_MAX_DIMS_ERROR.Args(dimCount, MAX_DIM_COUNT));
          ...
          {
             throw new SlimException(StringConsts.SLIM_ARRAYS_ARRAY_INSTANCE_ERROR + descr.ToString()+"': "+error.Message, error);
          }
          ...
          return instance;
                ...
                private static int quickParseInt(string str, ref int i, int len)
                {
                  ...
                  for(; i<len; i++)
                  {
                    ...
                    var d = c - '0';
                    if (d<0 || d>9) throw new SlimException(StringConsts.SLIM_ARRAYS_WRONG_ARRAY_DIMS_ERROR + str);
                    result *= 10;
                    result += d;
                  }
